...
int main()
{
    ...
    /* 4.5 - Enumerated types:
     ...
     Typedefs allow the programmer to create an alias for a data type, and use the aliased name instead of the actual type name. To declare a typedef
     ...
     Type defs allow you to change the underlying type of an obect without having to change lots of code. For example,
     if you're using a short to hold student ID's, but then you decided that you need a long instead, you'd have to comb
     through lots of code and replace short with long. i'd probably be difficult to figure out which shorts are used to
     hold ID's and which were used for other purposes
     
     with a typedef you, all you have to do is change typedef short studentID_t to typedef long studentID_t.
     Be cautious when changing the type of a typedef to a type in a different type family (e.g. from integer to a floating
     point value, or vice versa). The new type may have comparison or integer/floating point division issues that the old
     type did not have.
     
     Platform independent coding:
     
     One big adventage of typedefs is that they can be used to hide platform specific details. On some platforms an integer
     is 2 bytes, on others its 4. Thus, using int to store more than 2 types of information can be potentially dangerous
     when writing platform independent code.
     
     Because char, short, int, and long give no indication to their size, it is fairly common for cross platform programs
     to use typedefs to define aliases that include the type's size in bits. For example, int8_t would be an 8bit signed
     integer, int16_t a 16 bit signed integer, and int32_t an 32 bit signed integer. using typedef names in this manner
     helps prevent mistakes and makes it more clear about what kind of assumptions have been made about the size of the
     variable.
     
     in order to make sure each typedef type resolves to a type of the right size, typedefs pf this kind are typically
     used in conjunction with the preprocessor:
     ...
     On machines where integers are only 2 bytes, INT_2_BYTES can be #defined, and the program will be compiled with the
     top set of typedefs. On machines where integers are 4 bytes, leaving INT_2_BYTES undefined will cause the bottom set
     of typedefs to be used.  this way, int int8_t will resolve to a 1 byte integer, int16_t will resolve to a 2 byte
     integer, and int32_t will resolve to a 4byte integer using the combination of char, short, int, and long that is
     appropriate for the machine the program is being compiled on.
     
     In C++11 this is actually how the fixed width integers (like int8_t) were defined! As a side-effect of the fact that
     int8_t is actually a typedef of char, the following code acts somewhat unexpectedly.:
     
     #include <cstdint> // for fixed width integers
     #include <iostream>
     
     int main(){
     std::int8_t i (97); // int8_t is actually a typdef for char
     std::cout << i << std::endl;
     ...
     Although we have only dealt with simple data types so far, in advanced C++ you could see a variable and function
     declared like this:
     
     std::vector<std::pair<std::string, int> > pairlist;
     
     bool hasDublicates(std::vector<std::pair<::string, int> pairlist) {
        // some code here
     }
     
     Typing std::vector<std::pair<std::string, int> > everywhere you need to use that type can get cumbersome.
     its much easier to use a typedef:
     ...
     Don't worry if you dont know what std::vector, std::pair, or all these crazy angle brackets are yet. The only thing
     you really need to understand here is that typedefs allow you to take complex types and give them a simple name,
     which makes those types easier to work with and understand.
     
     Type aliases in C++11:
     
     Typedefs have a few issues. First, it's easy to foget whether the typename or type definition come first.
     which is correct?
     
     typedef distance_t double; // incorrect
     typedef double distance_t; // correct
     
     Second the syntax for typedefs gets ugly with more complex types (as we'll explore further in the section on function
     pointers).
     
     To help address these issues, in C++11 a new, improved syntax for typedefs has been introduced that mimics the way
     variables are declared. This syntax is called a type alias. A type alias introduces a name that can be used as
     ...
     Note that although the C++11 synxtax uses the "using" keyword, thsi is an overloaded meaning, and does not have
     anything to do with the using statements related to namespacing.
     
     This new syntax is cleaner for more advanced typedefing cases, and should be preferred if your compiler is C++11
     capable.
     ...
     1a) convert the int return value to a typedef named error_t using a typedef keyword. Include both the typedef
     ...
     1b) Convert the int return value to a typedef named error_t using the using keyword (C++11). Include both the typdef
     statement and the updated function prototype.
     ...
     There are many instances in programming where we need more than one variable in order to represent an object. For
     eexample, to represent yourself, you might want to store your name, birthday, height, weight, or any other number of
     characteristics about yourself. You could do so like this:
     ...
     However, you now have 6 independent variables that are not grouped in any way. If you wanted to pass information
     about yourself to a function, you'd have to pass each variable individually. Furthermore if you wanted to store
     information about someone else, you'd have to declare 6 more variables for each additional person! As you can see,
     this can quickly get out of control.
     
     Fortunately C++ allows us to create our own user-defined aggregate data types. An aggregate data type is a data type
     that groups multiple individual variables together. One of the simples affrefate data types is the struct. A struct
     (short for structure) allows us to group variables of mixed data types together into a single unit.
     
     Declaring and defining structs:
     
     Because structs are user-defined. we first have to tell the compiler what our struct looks like before we can begin
     using it. to do this, we declare our struct using the struct keyword. Here is an example of a struct declaration:
     ...
     This tells the compiler that we are defining a struct named Employee. The employee struct contains 3 variables
     inside of it: a short named id, an int named age, and a double named wage. These variables that are part of the
     struct are called members (or fields). Keep in mind that Employee is just a declaration -- even though we are
     telling the compiler that the struct will have member variables, no memory is allocated at this time.
     By convention, struct names start with a capital letter to distinguish them from variable names.
     
     In order to use the Employee struct, we simply declare a variable of the type Employee:
     
     Employee joe; // strict Employee is capitalized, variable joe is not.
     
     This defines a variable of type Employee named joe. As with normal variables, defining a struct variable allocates
     ...
     In order to access the individual members, we use the member selection operator (which is a period). Here is an
     example of using the member selection operator to initialize each member variable:
     
     Employee joe; // create an Employee struct for joe
     joe.id = 64; // assign a value to member id within struct joe.
     joe.age = 32; // assign a value to member age within struct joe.
     joe.wage = 24.15; // assign a value to member wage within struct joe
     
     Employee frank; // create an Employee struct for Frank
     frank.id = 15; // assign a value of 15 to member id within struct frank
     frank.age = 28; // assign a value of 28 to member age within struct frank
     ...
     In the aboe example, it is very easy to tell which member variables belong to joe and which belong to frank
     this provides much higher level of organization than individual members have the same names, this provides
     consistenct across multiple variables of the same struct type.
     ...
     ++joe.age // use pre.increment to increment joes age by 1
     ...
     If the initializer list does not contain an initializer for some elements, those elements are initialized to a
     default value ( that generally corresponds to the zero state for that type). In the above example, we see that
     frank.wage gets default initialized to 0.0 because we did not specify an explict initialization value for it.
     
     C++11/14: Non-static member initialization:
     
     Starting with C++11, its possible to give non-static (normal) struct members a default value:
     ...
     In c++11 the non-static member initialization syntax is incompatible with the initializer list and uniform
     initialization syntax.
     
     So in C++11 you have to decide wether to use non-static member initialization or uniform initialization. Uniform
     initialization is more flexible, so we recommend sticking with that one.
     
     In C++14 this restriction was lifted and moth can be used.
     
     We talk about what static members are in chapter 8.
     
     Assigning to structs:
     
     in C++11 you can assign values to structs members using an initializer list:
     ...
     A big adventage of using structs over individual variables is that we can pass the entire struct to a function
     that needs to work with the members:
     ...
     // Print franks information
     printInformation(frank);
     
     return 0;
     }
     
     A Function can also retyrn a struct, which is one of the few ways to have a function return multiple variables:
     ...
     In this case, if we wanted to know what the CEO's salary was, we sunply use the member selection operator twice:
     myCompany.CEO.wage;
    ...
    Typically, the size of a struct is the sum of the size of all its members, but not always!
     ...
     Consider the Employee struct. On many platforms, a short is 2 bytes, an int is 4 bytes, and a double is 8 bytes,
     so we'd expect Employee to be 2 + 4 + 8 = 14 bytes. To find out the exact size of Employee, we can use the sizeof
     operator:
     ...
     int main(){
     std::cout << "The sizr of Employee is " << sizeof(Employee) << "\n";
     ...
     It turns out we can only say that the size of a struct will be at least as large as the size of all the variables
     it contains. But it could be larger! For performance reasons, the compiler will sometimes add gaps into structures
     (called padding).
     
     In the Employee struct above, the compiler is incisibly adding 2 bytes of padding after member id, making the size
     of the structure 16 bytes instead of 14. The reason it does this is beyond the scope of this tutorial
     (you can read about it in wikipedia "data structure alignment" but this is not required to understand structers on c++)
     
     Accessing structs across multiple files:
     
     Because struct declarations do not take any memory, if you want to share a struct declaration across in multiple
     files (so you can insatntiate variables of that struct type in multiple files), put that struct declaration in a
     header file and #include that header file anywhere you need it.
     
     Struct variables are subject to the same rules as normal variables. Consequently, to make a struct variable
     accessible across multiple files, you can use the extern keyword to do so.
     
     Final notes on structs:
     
     Structs are very important in C++, as understanding structs is the first major step towards object oriented
     programming! Later on in these tutorials, you'll learn about another aggregate data type called a class, which is
     built on top of structs. Understanding structs well will help make the transition to classes that much easier.
     
     The structs introduced in this lesson are sometimes called plain old data structs (or POD structs) since the members
     are all data (variable) members. In the future (when we discuss classes) we'll talk about other kinds of members.
     ...
     2) Create a struct to hold a fraction. The struct should have an integer numerator and an integer denominator
     member. Declare 2 fraction variables and read them in from the user. Write a function called multiply that takes
     both fractions, multiplies them together, and prints the result out as a decimal number. You do not need to reduce
     the fraction to its lowest terms.
     */
    
    // 1)
    
    // Getting values from user for advertising
    
    // In the tutorial, getting the values was written into its own function.
    
    Advertising ads;
    std::cout << "Enter how many ads was shown: ";
    std::cin >> ads.adsShown;
    std::cout << "Enter the percentage of shown ads clicked: ";
    std::cin >> ads.percentageOfAdsClicked;
    std::cout << "Enter how much was earned per click (on average): ";
    std::cin >> ads.earnedPerClick;
    
    // Passing advertising struct to struct to a function that prints each of the values, and then calculates earnings.
    
    calculateEarnings(ads);
